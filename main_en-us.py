...
save = False
font_size=15
filetype = [('Text File', '*.txt'), ('Python File', '*.py *.pyw'), ('C/C++ File', '*.c *.cpp *.cs'), ('Batch File', '*.cmd *.bat'),
            ('HTML File', '*.htm *.html'), ('Reg File', '*.reg'), ('All File', '*.*')]
...
def save_as():
    ...
    path = asksaveasfilename(defaultextension='.txt', filetypes=filetype, title='Save As')
    with open(path, mode='w', encoding='utf-8') as sa:
        sa.write(text1.get(0.0, END))
    save = True
    win.title('Notepad-' + path + '-1.2-release1')
    ...
    with open('data/theme.ini', 'r+', encoding='utf-8') as th:
        if th.read() == '':
            ...
            if th.read() == 'theme=1':
                ...
                v1 = 2
    e2 = Radiobutton(root, variable=v1, value=1, text='Light', command=use_light_theme)
    e2.grid(row=1, column=0)
    e3 = Radiobutton(root, variable=v1, value=2, text='Dark', command=use_dark_theme)
...
def open_file():
    save = True
    s = askopenfile(defaultextension='.txt', filetypes=filetype, title='Open')
    path = s.name
...
def search(searchName,Searchwz='Baidu',t=0):
    if t==1:
...
def mainwindow():
    ...
    size.set('15')
    b3 = Button(w2, text='Apply', command=lambda: apply(butt.get(), size.get(), c))
    c = IntVar()
...
def command_run(command):
    ...
    if command == 'set-theme' or command == '/set-theme' or command == '--set-theme' or command == '-ST':
        ...
        if not (argv[2]=='dark' or argv[2]=='light'):
            raise SyntaxError('Error:You have entered a theme that does not exist. Please enter "dark" or "light"')
            #print('Error:You have entered a theme that does not exist. Please enter "dark" or "light"')
        if argv[2]=='light':
            win.iconbitmap('icons/dark.ico')
        else:
            win.iconbitmap('icons/light.ico')
...
def update_log():
    logtitle = '''1.2-pre1 Update '''
    log = '''Update:
\t1.Added the function of running Python code
'''
    win3 = Tk()
    win3.title('Update Log')
    win3.iconbitmap('icons/log.ico')
    win3.resizable(False, False)
    ul1 = Label(win3, text=logtitle, font='Consolas 19 bold')
    ul1.grid(row=1,column=0)
    ul2 = Label(win3, text=log, font='Consolas 14')
    ul2.grid()
    win3.mainloop()
...
def command_help():
    ...
    commandHelp = '''open、-O、--open、/open:How to use automatic file opening: python main.py --open {filePath} [Encoding]
FilePath: Open file path (can be relative or absolute) Encoding: Open file uses encoding (default UTF-8)

/help、--help、-H、help:Description window  usage method:python main.py --help

/set-theme、--set-theme、-ST、set-theme:Start Notepad with a custom theme  usage method:python main.py --set-theme {ThemeName}
ThemeName:Theme name(Optional "light" or "dark")


{ValueName}:Required              [ValueName]:Optional
    '''
    if len(argv)>1:
        if argv[1] == 'help' or argv[1] == '/help' or argv[1] == '--help' or argv[1] == '-H':
            print(commandHelp)
    Label(w3,text='Command Help',font='Consolas 17 bold').grid()
    Label(w3,text=commandHelp,font='Consolas 12').grid(row=1,column=0)
    win.mainloop()


def set_icon():
    iconPath=askopenfilename(filetypes=[('Icon File','*.ico'),('All File','*.*')],title='Select Icon File')
...
def add_tag_window(tagstart='',tagend=''):
    r2=Toplevel()
    r2.title('Tag Sitting')
    r2.resizable(False,False)
    Label(r2,text='Enter the start position (format: row. column, starting from 1)').grid()
    entr1=Entry(r2)
    entr1.grid(row=1,column=0,columnspan=2)
    Label(r2, text='Enter the end position (format: row. column, starting from 1)').grid(row=2,column=0)
    entr2 = Entry(r2)
    entr2.grid(row=3, column=0,columnspan=2)
    entr1.insert(0,tagstart)
    entr2.insert(0, tagend)
    Label(r2, text='Enter background color').grid(row=4, column=0)
    entr3=Entry(r2)
    ...
    Label(r2, text='Enter foreground color').grid(row=5, column=0)
    entr4 = Entry(r2)
    entr4.insert(0,'red')
    entr4.grid(row=5, column=1)
    Label(r2, text='Enter the tag name').grid(row=6, column=0)
    entr5 = Entry(r2)
    entr5.grid(row=6,column=1)
    entr5.insert(0, 'tag1')
    addB=Button(r2,text='Add Tag',command=lambda:add_tag(entr5.get(),entr1.get(),entr2.get(),entr3.get(),entr4.get()))
    addB.grid(row=7,column=0,ipadx=30,ipady=7)
    remB = Button(r2, text='Remove Tag',command=lambda:remove_tag(entr5.get()))
...
menu_right.add_cascade(label='Copy', command=copy)
menu_right.add_cascade(label='Paste', command=paste)
menu_right.add_cascade(label='Select All', command=select_all)
menu_right.add_cascade(label='Delete',command=lambda:text1.delete(SEL_FIRST,SEL_LAST))
menu_right.add_cascade(label='Search', command=lambda:search(text1.get(SEL_FIRST,SEL_LAST),t=1))
menu_right.add_cascade(label='Tag Sitting',command=lambda:add_tag_window(tagstart=SEL_FIRST,tagend=SEL_LAST))
menu_right.add_separator()
menu_right.add_command(label='Run Batch File',command=run_batfile)
...
menu1_1.add_command(label='New File', command=new)
menu1_1.add_command(label='Save', command=save)
menu1_1.add_command(label='Save As', command=save_as)
menu1_1.add_command(label='Open', command=open_file)
menu1_1.add_separator()
menu1_1.add_command(label='Exit', command=win.quit, accelerator='Alt+F4')
menu2_1=Menu(menu1,tearoff=False)
menu1.add_cascade(label='Edit',menu=menu2_1)
menu2_1.add_command(label='Copy', command=copy, accelerator='Ctrl+C')
menu2_1.add_command(label='Cut', command=cut, accelerator='Ctrl+X')
menu2_1.add_command(label='Paste', command=paste, accelerator='Ctrl+V')
menu2_1.add_command(label='Select All', command=select_all, accelerator='Ctrl+A')
menu2_1.add_command(label='Delete',command=lambda:text1.delete(SEL_FIRST,SEL_LAST),accelerator='Backspace')
menu2_1.add_command(label='Tag Sitting', command=add_tag_window)
menu2_1.add_separator()
menu2_1.add_command(label='Undo', command=undo)
menu2_1.add_command(label='Redo', command=redo)
menu3_1 = Menu(menu1, tearoff=False)
menu1.add_cascade(label='Sitting', menu=menu3_1)
menu3_1.add_command(label='Font', command=mainwindow)
menu3_1.add_command(label='Theme', command=themeWindow)
menu3_1.add_command(label='Run Batch code', command=run_batfile)
menu3_1.add_command(label='Run Python code',command=run_pyfile)
...
#menu4_1.add_command(label='Restore window default size',command=nor_size)
menu4_1.add_command(label='Set Window Icon',command=set_icon)
menu5_1 = Menu(menu1, tearoff=False)
menu1.add_cascade(label='Search', menu=menu5_1)
menu6_1 = Menu(menu1, tearoff=False)
menu5_1.add_command(label='Baidu',command=lambda:search(searchName=text1.get(SEL_FIRST,SEL_LAST),Searchwz='Baidu'))
menu5_1.add_command(label='Bing',command=lambda:search(searchName=text1.get(SEL_FIRST,SEL_LAST),Searchwz='Bing'))
menu5_1.add_command(label='Sougou',command=lambda:search(searchName=text1.get(SEL_FIRST,SEL_LAST),Searchwz='Sougou'))
menu5_1.add_command(label='360',command=lambda:search(searchName=text1.get(SEL_FIRST,SEL_LAST),Searchwz='360'))
menu5_1.add_command(label='Bilibili',command=lambda:search(searchName=text1.get(SEL_FIRST,SEL_LAST),Searchwz='Bilibili'))
menu5_1.add_command(label='Baidu Bar',command=lambda:search(searchName=text1.get(SEL_FIRST,SEL_LAST),Searchwz='Baidu Bar'))
menu1.add_cascade(label='About', menu=menu6_1)
menu6_1.add_command(label='About Notepad', command=about)
menu6_1.add_command(label='Update Log', command=update_log)
menu6_1.add_command(label='Command Help', command=command_help)
